...
export const QuestionnairePanel = ({ applicantId, partId, semester }: QuestionnairePanelProps) => {
  ...
  const activeMembers = activeMembersResponse.members;
  const isQuestionnaireDisabled = hasSubmittedInterviewEvaluation({
    evaluatorStatuses,
    myEvaluation,
  });
  const isSharedQuestionDisabled = rubric.isLocked;
  const {
    control,
  ...
  const onSubmit: SubmitHandler<QuestionnaireFormValues> = (values) => {
    if (isQuestionnaireDisabled) {
      return;
    }
...
interface HasSubmittedInterviewEvaluationParams {
  evaluatorStatuses: InterviewEvaluatorStatuses;
  myEvaluation: MyInterviewEvaluation;
}

const hasSubmittedInterviewEvaluation = ({
  evaluatorStatuses,
  myEvaluation,
}: HasSubmittedInterviewEvaluationParams) =>
