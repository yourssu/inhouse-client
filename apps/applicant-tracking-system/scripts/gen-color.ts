...
import { indent, log, sourceBasePath, writeFileEnsureDirectorySync } from './utils';

const stylesheetsBasePath = `${sourceBasePath}/styles`;
const variablesFileName = 'colors.css';
const variablesFilePath = `${stylesheetsBasePath}/${variablesFileName}`;
const resultPath = `${stylesheetsBasePath}/__generated__/colors.gen.css`;
const moudleResultPath = `${stylesheetsBasePath}/__generated__/colors.gen.ts`;
...
function readColorVariableNames(file: string, prefix: string) {
  const escapedPrefix = prefix.replace(new RegExp('[-/\\\\^$*+?.()|[\\]{}]', 'g'), '\\$&');
  const variables = file.match(new RegExp(`${escapedPrefix}\\s*{([^}]*)}`, 's'))?.[1];

  if (!variables) {
    log.error(`${variablesFileName} 파일 내에 ${prefix} { ... } 블록을 찾을 수 없어요.`);
    return [];
  }
...
function getGeneratedResult(tokens: string[]) {
  const comment = '/* scripts/gen-color.ts에 의해서 자동으로 채워져요. */\n';
...
function main() {
  ...
  const variables = uniq([
    ...readColorVariableNames(varFile, "[data-theme='light']"),
    ...readColorVariableNames(varFile, "[data-theme='dark']"),
    ...readColorVariableNames(varFile, ':root'),
  ]);
