...
- 현재 HEAD가 의도한 base와 어떤 관계인지 확인한 뒤 브랜치를 생성하거나
  전환한다. 기존 브랜치를 reset하거나 커밋을 암묵적으로 옮기지 않는다.
- `inhouse-skill`과 작업 영역의 참조 문서를 적용해 영향받은 workspace의
  type, lint, format, test, build 중 필요한 검증을 선택한다.
- 실행한 명령과 종료 결과를 기록한다. 실행하지 않은 검증을 통과로 쓰지 않고,
  기존 오류와 이번 변경의 오류는 증거가 있을 때만 구분한다.
