...
globs: src/components/**/*.{ts,tsx}, src/styles/**/*.css, src/utils/tw.ts
---
...
```tsx
// src/components/_ui/Table/Th.tsx
...
- 컴포넌트 내부에 다양한 크기(`size`), 모양(`variant`), 색상(`color`) 등을 설정할 때는 무조건 `tailwind-variants`의 `tv()`(`src/utils/tw.ts`에서 import된 커스텀 tv)를 이용합니다.
...
```tsx
// src/components/_ui/Badge/index.tsx
...
- 디자인 시스템 테마 토큰은 `src/styles/colors.css` 파일에 사용자가 직접 정의하여 사용합니다.
- 임의로 하드코딩된 RGB/Hex 값을 사용하지 마세요. `src/styles/__generated__/colors.gen.css` 에 명시된 컬러 토큰들만 사용해야 합니다. (e.g. `text-neutral`, `bg-buttonPrimaryBackground` 등 실제 `colors.css`에 정의된 시맨틱/시스템 컬러 활용).
...
- 이 프로젝트는 `[data-theme='dark']` 속성을 기반으로 다크 모드를 판별합니다.
